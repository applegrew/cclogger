...
#     //
#    '
# AppleGrew Mod see https://github.com/coleifer/peewee/pull/91
...
class DecimalField(Field):
    ...
    def field_attributes(self):
        return {
            ...
            'always_float': False, # By AppleGrew
        }
    ...
    def python_value(self, value):
        if value is not None:
            if self.attributes['always_float']:  # By AppleGrew [
...
class DateTimeField(Field):
    ...
    def db_value(self, value): # AppleGrew mod start [
        if value and isinstance(value, datetime.datetime):
            if value.tzinfo is not None:
                value = value.replace(tzinfo=None) # Making it naive since that is not supported by DB.
        return value        # AppleGrew mod ends ]
